...
#'
#' @return dataframe of time on study with a column for SiteID, SubjectID, first study date, last study date
#' and a TimeOnStudy column giving time on study in days
#'
#' @import dplyr
...
TimeOnStudy <- function(dfVisit = NULL,
                        dfStud = NULL,
                        dtSnapshot = NULL) {
  ...
  dfVisitRange <- dfTOS %>%
    ...
    summarise(
      FirstStudyDate = min(.data$RECORDDATE, na.rm = T),
      LastStudyDate = if_else(
        first(.data$SUBJID) %in% completedIDs,
        as.Date(max(.data$RECORDDATE, na.rm = T)),
        as.Date(dtSnapshot)
      )
    ) %>%
    mutate(TimeOnStudy = as.numeric(difftime(.data$LastStudyDate, .data$FirstStudyDate, units = "days") + 1)) %>%
